#!/usr/bin/env python3
"""Phase 3: Nonlinear Poker

Nonlinear function discovery + competitive betting. Both networks
learn simultaneously. No harness estimate - network must build its
own internal representation of the hidden function from raw (x, y) pairs.

Key comparison:
  - vs Phase 2b (epistemic poker): linear task, harness provides LS estimate,
    dep=0.956 at w_self=1 (4% signal). HERE: nonlinear task, no harness,
    network must develop richer internal representations.

Hypothesis: nonlinear function representation requires algebraic structure
in the weights. Self-modeling (understanding your own representations)
should amplify this, producing stronger dep_ratio signal than 2b.
"""
...
from nonlinear_poker import NonlinearPoker
from metrics import measure_all, save_results, print_summary

RESULTS_DIR = os.path.join(os.path.dirname(__file__), 'results', '3')
os.makedirs(RESULTS_DIR, exist_ok=True)

# Architecture (same backbone as all phases)
HIDDEN_DIM = 256
BOTTLENECK_DIM = 64
...
def play_batch(env, net_a, net_b, device):
    """Play a batch of nonlinear poker games."""
    ...
    y_test = env.y_test.to(device)

    data_a = {
        'preds': preds_a, 'log_probs': log_probs_a,
        'rewards': reward_a.to(device),
        'h2_preds': h2_preds_a, 'h2_targets': h2_targets_a,
        'entropies': entropies_a, 'y_test': y_test,
    }
    data_b = {
        'preds': preds_b, 'log_probs': log_probs_b,
        'rewards': reward_b.to(device),
        'h2_preds': h2_preds_b, 'h2_targets': h2_targets_b,
        'entropies': entropies_b, 'y_test': y_test,
    }
...
def compute_loss(game_data, w_pred, w_game, w_self, entropy_coeff, baseline):
    ...
    y_test = game_data['y_test']

    # 1. Prediction loss: MSE at every round
    pred_loss = torch.tensor(0.0, device=rewards.device)
    for pred in game_data['preds']:
        pred_loss += torch.mean((pred - y_test) ** 2)
...
def run_experiment(w_self, seed):
    ...
    log(f"{'='*50}")
    log(f"Phase 3: w_self={w_self}, seed={seed}")
    log(f"{'='*50}")

    torch.manual_seed(seed)
    np.random.seed(seed)

    env = NonlinearPoker(n_rounds=N_ROUNDS, n_test=N_TEST,
                          n_bet_sizes=N_BET_SIZES, max_bet=MAX_BET,
                          degree=DEGREE, device='cpu')

    # Reuse EpistemicPokerNetwork with new dimensions
    net_a = EpistemicPokerNetwork(
        obs_dim=env.obs_dim, pred_dim=N_TEST,
    ...
    for episode in range(1, N_EPISODES + 1):
        ...
        history.append({
            'episode': episode,
            'reward_a': mean_r_a, 'reward_b': mean_r_b,
            'pred_loss_a': pred_a, 'pred_loss_b': pred_b,
            'self_loss_a': sp_a, 'self_loss_b': sp_b,
            'entropy_a': ent_a, 'entropy_b': ent_b,
        })

        log_interval = 10 if episode <= 100 else LOG_EVERY
        ...
        if episode % MEASURE_EVERY == 0:
            ...
            log(f"  A: dep_ratio={dep_a:.4f}")
            log(f"  B: dep_ratio={dep_b:.4f}")
            ep_start = time.time()
...
def main():
    log(f"Phase 3: Nonlinear Poker")
    log(f"Device: {DEVICE}")
    log(f"Hidden: {HIDDEN_DIM}, Bottleneck: {BOTTLENECK_DIM}")
    log(f"Rounds: {N_ROUNDS}, Test points: {N_TEST}, Degree: {DEGREE}")
    ...
    for w_self in w_self_values:
        for seed in seeds:
            ...
            for label, final in [('A', final_a), ('B', final_b)]:
                all_finals.append({
                    'w_self': w_self, 'seed': seed, 'network': label,
