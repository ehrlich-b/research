...
Non-degeneracy of~$B$ ensures that every model effect~$b$ is
correlated with some body effect~$a$ through the self-modeling
isomorphism~$\varphi$.  This is the conceptual reason product
measurements are tomographically complete: no body--model correlation
is invisible to product-form observations.  The formal proof below
establishes the same conclusion via state separation, which requires
only the composite axioms (C1)--(C4) and avoids working with the
bilinear form on the composite directly.
\end{remark}
...
\emph{Upper bound.}
By minimality (Assumption~\ref{ass:minimal-composite}), $V_{BM}$
contains no structure beyond what is forced by (C1)--(C4) and the
product-form SP\@.  The product effects span a $d^2$-dimensional
subspace, and the SP preserves this subspace (product effects map to
product effects under~\eqref{eq:product-sp}), so minimality gives
$\dim(V_{BM}) \le d^2$.
\end{proof}
