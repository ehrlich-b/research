...
\item[\emph{(C4)}] \textbf{Non-signaling.}
  ...
  $\omega_B(a) \coloneqq \omega(a\otimes\id_M)$ and
  $\omega_M(b) \coloneqq \omega(\id_B\otimes b)$ are well-defined
  states, independent of what is measured on the other subsystem.
\end{enumerate}
\end{definition}

\noindent
By Definition~\ref{def:self-modeling-system}\ref{sms:minimal},
...
\phantomsection\label{ass:minimal-composite}
...
\begin{equation}\label{eq:product-sp}
  \seqp{(a \otimes b)}{(c \otimes d)}
  = (\seqp{a}{c}) \otimes (\seqp{b}{d}),
\end{equation}
where $\seqp{\cdot}{\cdot}$ on the right-hand side denotes the
...
$\seqp{a}{c}=\seqp{c}{a}$ and $\seqp{b}{d}=\seqp{d}{b}$.

The inheritance of S4 (orthogonality symmetry) is more subtle. If
$\seqp{(a\otimes b)}{(c\otimes d)} = 0$, then
$(\seqp{a}{c})\otimes(\seqp{b}{d}) = 0$. In an order unit space, states
separate points (Alfsen--Shultz~\cite{AlfsenShultz2003}, Theorem~1.23):
for product states $\omega_B\otimes\omega_M$, the evaluation
$\omega_B(\seqp{a}{c})\,\omega_M(\seqp{b}{d}) = 0$ for all
$\omega_B,\omega_M$ forces either $\seqp{a}{c} = 0$ or $\seqp{b}{d} = 0$.
By the factor-level S4, the corresponding reverse products vanish, giving
$\seqp{(c\otimes d)}{(a\otimes b)} = 0$.
\end{proof}
...
this product carries EJA structure.  State separation
(Theorem~\ref{thm:local-tomo}) then shows that the product effects
$\{a_i \otimes b_j\}$ are linearly independent, giving
$\dim(V_{BM}) \ge \dim(V_B) \cdot \dim(V_M)$.  Minimality
(Definition~\ref{def:self-modeling-system}\ref{sms:minimal}) supplies the upper bound,
yielding $V_{BM} = V_B \otimes V_M$.
\end{remark}

%--------------------------------------------------------------------
\subsection{Local Tomography}\label{subsec:lt-proof}

The key step is showing that the composite is locally tomographic:
product measurements suffice to determine all joint states.
Conceptually, faithful self-modeling ensures that body--model
correlations carry full information (Proposition~\ref{prop:nondeg}
below); the formal proof then uses state separation and minimality.

\noindent
By Definition~\ref{def:self-modeling-system}\ref{sms:simple},
$V$ has no nontrivial direct-sum decomposition into order-unit
subspaces.  Equivalently, once the EJA structure is established, $V$
is simple.  This clause is needed for the non-degeneracy of the
trace form below.%
\phantomsection\label{ass:simple-eja}
...
\begin{remark}[Role of the correlation form]\label{rem:corr-role}
Non-degeneracy of~$B$ provides intuition: every model effect~$b$ is
correlated with some body effect~$a$ through the self-modeling
isomorphism~$\varphi$, so no body--model correlation is invisible to
product-form observations.  The formal proof of local tomography
below does not use~$B$ directly; it proceeds via state separation
(lower bound) and minimality (upper bound).  The correlation form
serves as a consistency check that the self-modeling isomorphism
supplies enough correlations for product measurements to be
tomographically complete, but it is not a step in the proof.
\end{remark}
...
(Definition~\ref{def:self-modeling-system}\ref{sms:minimal}) eliminates this sector by
