...
The formalization comprises 10 files (${\sim}2{,}400$ lines) and
produces zero \texttt{sorry} declarations.  Four axioms are used:
(1)~the self-modeling bridge, which encapsulates the sequential
product construction of Sections~3--4;
(2)~the spectral Jordan identity (van de Wetering 2019, \S4);
(3)~local tomography
(Definition~\ref{def:self-modeling-system}\ref{sms:minimal}---the
minimality-to-dimension argument of \Cref{thm:local-tomo}); and
(4)~the $C^{*}$-promotion theorem (van de Wetering 2019,
Theorem~3).  Axioms (2)--(4) are published theorems; axiom~(1)
bridges the paper's novel construction to the formalized framework.
