...
asks only that the system can simulate \emph{itself}.  A companion
paper~\cite{Ehrlich2026measure} constructs an experiential measure on
isomorphism classes of self-modeling dynamical structures, where the
density functional $\rho = I(B;M)(1 - I/H)$ peaks at intermediate
self-modeling fidelity.  The present result sharpens that framework:
$\rho$ selects self-modelers, and the algebraic consequences of
self-modeling (this paper) produce quantum mechanics by theorem.
The connection to the consciousness literature (where self-modeling
has been studied under different names) is suggestive but not
load-bearing: our result is a mathematical theorem about order unit
spaces, independent of interpretive commitments.
\end{remark}
