...
$V \cong M_n(\mathbb{C})^{\mathrm{sa}}$ for some $n \ge 2$.  The case
$n = 1$ ($V \cong \mathbb{R}$) is excluded by the nontriviality
condition in
Definition~\ref{def:self-modeling-system}\ref{sms:finite}: a
one-dimensional OUS has no nontrivial projective units, so the
Peirce structure is vacuous and the self-modeling cycle produces no
nontrivial sequential product.  This is
...
Neither of these two broader theorems is used in the derivation
(unlike the specific dimension-counting and no-composite results
of~\cite{BarnumWilce2014} and~\cite{BarnumUdudeckVdW2020} invoked
in Section~\ref{subsec:exclusion} above); both serve as independent
cross-checks that the conclusion is robust.
\end{remark}
...
Let $(V, \varphi, V_{BM})$ be a self-modeling system
(Definition~\ref{def:self-modeling-system}).  Then:
\begin{enumerate}
\item $V \cong M_n(\mathbb{C})^{\mathrm{sa}}$ for some $n \ge 2$;
\item the sequential product is the L\"uders product
  $\seqp{a}{b} = \sqrt{a}\, b\, \sqrt{a}$;
\item $M_n(\mathbb{C})$ carries the conjugate-transpose involution
  $X^* = X^\dagger$.
\end{enumerate}
\end{theorem}

In particular, the complex field, the Jordan product, and the
C*-involution are all consequences of the self-modeling definition
(Definition~\ref{def:self-modeling-system}).  Local tomography follows
from the internal-accessibility condition~\ref{sms:minimal}; the
complex field and C*-structure then follow from the classification
machinery.
...
\item Dimension counting excludes real and quaternionic types;
  published no-composite results exclude spin factors
  (\cite{BarnumWilce2014}, Thm.~4.1) and the Albert algebra
  (\cite{BarnumUdudeckVdW2020}, Thm.~1)
