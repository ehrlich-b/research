...
(Claim~\ref{claim:rhoJ}) is now a theorem, with falsifiable predictions
(Sec.~\ref{subsec:predictions}).
...
Gap & Severity & Status \\
\midrule
Smooth-manifold dynamics & Real & GST identification, not a derivation \\
Gap~C (complexification) & Real & Evolutionary argument; not a thm \\
Minimal composite assumption & Significant & Load-bearing; not derived \\
Finite-dimensional assumption & Moderate & Natural for subsystems \\
\bottomrule
\end{tabular}
\end{table}

The GR step is now an \emph{identification} with the exceptional GST
entry, not a lattice derivation; its open bridge is smooth-manifold
dynamics, shared by every program that builds gravity on an algebraic
or discrete substrate.
