...
$\hthree$ = exceptional MESGT & Identification & Paper~6 (GST) \\
Einstein term $-R/2$ & Identified & Paper~6 (smooth dyn.\ open) \\
...
measure $\rho$ (Claim~\ref{claim:rhoJ}) selects a position on this
...
\item \textbf{Lean formalization.}  Paper~5's downstream derivation
  chain (EJA classification $\to$ local tomography $\to$ type
  exclusion $\to$ $C^*$-promotion) compiles in Lean~4 with Mathlib,
  with zero \texttt{sorry} placeholders and four axioms (three
  published results, one bridge axiom connecting the novel construction
  to the downstream machinery).  Eliminating the bridge axiom ---
  proving S1--S7 for the corrected product directly in Lean --- is in
  progress and would upgrade the entire chain to machine-verified.
\end{enumerate}
