...
This is the sense in which Level~IV is a direct consequence of radical
relativity rather than an independent axiom.  It posits nothing.  It
is what remains when you stop positing.  We call this a ``tautology''
in the chain table (Table~\ref{tab:chain}) to emphasize that it adds
no new content beyond the premise.  Whether the inference from ``no
privilege'' to ``all exist'' is logically airtight or merely the
natural default is a philosophical question we do not claim to have
settled; the derivation chain treats it as the working assumption.
...
the self-reference density $\rho$ (the Observable-side weight) marks
self-modelers, and experience---whose magnitude is the Stream
functional $\Phi$---lives where the self-modeling \emph{runs} with
$\rho > 0$.  From the idealist direction: experience is
