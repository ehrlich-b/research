...
\subsection{The self-reference density (the Observable-side weight)}
...
Paper~1~\citep{ehrlich2026measure} defines the self-reference density
\begin{equation}
\label{eq:rho}
...
\item $\rho > 0$ at intermediate fidelity: imperfect but active
  self-modeling.
\end{enumerate}
$\rho$ is the instantaneous Observable-side \emph{weight}; the magnitude
of experience is the Stream functional $\Phi$ of Paper~1, which vanishes
wherever $\rho$ does.  Rocks, thermostats, and unstructured noise score $\rho = 0$.  Cells
...
(Sec.~\ref{sec:why-qm}), $\rho$ resolves to a function of the
algebra's invariants (see Sec.~\ref{sec:why-this-physics}).  You do
not need to postulate it separately.
...
is what self-models model.  This has the form $A \to B \to C \to A$,
which invites the charge of circularity.  The response: fixed-point
characterizations are not circular.  The claim is that self-modeling,
the algebra, and $\rho$ are \emph{mutually consistent} at exactly one
point (within $\hthree$, the $\rhoJ$ uniqueness theorem makes this
precise: Sec.~\ref{sec:why-this-physics}).  The existence question ---
proving that the fixed point exists and is unique in the general
setting --- is open.
...
\begin{remark}[Where the gap lives]
The model is an \emph{Observable} record---compressed, and recoverable
by a third party.  The experience is $\Phi$, a functional of the
self-modeling \emph{trajectory} (Paper~1), whose magnitude lives in
path-space content that the Observable record provably underdetermines.
The explanatory gap between physical description and phenomenal
experience is therefore an \emph{inaccessibility}, not a difference in
$\rho$-levels: the live research target
(\texttt{phi-inaccessibility-program}) is that no self-modeler can
recover its own $\Phi$ from any Observable record it can persist---a
G\"odel/Tarski-type self-reference limit, stated but not yet proved.
$\rho$ is the Observable-side weight, not the experience; the gap is
