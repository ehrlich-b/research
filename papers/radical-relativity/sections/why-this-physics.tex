...
\subsection{General relativity by identification with exceptional supergravity}
\label{subsec:gr}

Paper~6~\citep{ehrlich2026gr} reads gravity off $\hthree$ by
\emph{identification}, not by an independent derivation.  The cubic
norm $\det(X)$ is the unique $\Ffour$-invariant cubic (Springer).
Within the Gunaydin--Sierra--Townsend classification of $N{=}2$
Maxwell--Einstein supergravity, the algebraic data of $\hthree$ match
the \emph{exceptional} entry, whose bosonic Lagrangian contains the
Einstein--Hilbert term $-R/2$; $\det(X)$ is its prepotential, fixing
the matter--gravity couplings.  An observer's Peirce decomposition
$27 = 1 \oplus 16 \oplus 10$ sends the complement
$V_0 = h_2(\mathbb{O})$ through the $C^*$-bottleneck to
$h_2(\mathbb{C}_u) \cong \mathbb{R}^{3,1}$, a 4-dimensional algebraic
Minkowski slice.

\textbf{What this is and is not.}  This is an \emph{identification
theorem}---$\hthree$'s cubic norm \emph{is} the exceptional GST
prepotential, so the matter--gravity couplings are determined by
$\det(X)$---conditional on the shared assumption that the algebraic
spacetime carries smooth field-theoretic dynamics on a manifold.  It
is \emph{not} an independent derivation of GR.  The earlier
lattice/Jacobson route was abandoned: the lattice is a modeling
choice, and an attempted Weinberg-style forcing of $-R/2$ failed (the
soft-graviton hypotheses are necessary, not sufficient).
Smooth-manifold dynamics is the principal open bridge.
...
The universe's self-modeling algebra is $\hthree$, conditional on the anthropic observer-grade restriction of Paper~7.
\end{claim}
...
\label{rem:converse}
Step~5 of the non-composability argument uses the converse of Paper~5's
main theorem: every $\Mnsa{n}$ admits a faithful self-model.  The
construction is explicit: take $V_M = V_B = \Mnsa{n}$, $\varphi =
\mathrm{id}$, and equip the composite with the standard quantum tensor
product $\Mnsa{n} \otimes \Mnsa{n} = \Mnsa{n^2}$.  For complex matrix
algebras, the minimal and maximal composites
coincide~\citep{bgw2020}, so the minimal-composite assumption~(A3) is
automatically satisfied.  Together with Paper~5's forward direction
(self-modeling forces $\Mnsa{n}$), this gives: \emph{a
finite-dimensional spectral order-unit space admits a faithful
self-model if and only if it is $\Mnsa{n}$ for some $n \ge 2$.}
\end{remark}
...
\label{claim:rhoJ}\label{rem:rho-h3o}
...
equilibrium.  Their product is the unique minimal $\Ffour$-invariant
\emph{self-reference density} (the Observable-side weight in the
phenomenological measure $\Phi$ of Paper~1, not experience itself): (three-way correlation) $\times$ (thermodynamic
disequilibrium).
