...
The first half of the derivation argues that the observer's algebraic
nature---specifically, the fact that self-modeling forces a C*-algebra
structure---motivates complexification of the Peirce half-space of the
exceptional Jordan algebra, canonically upgrading the symmetry from
$\Spin{9}$ to $\Spin{10}$ and identifying the 16-dimensional
spinor representation that will carry one generation of fermions.
The complexification step is a physical argument, not a formal
theorem; its formalization is an open problem (Gap~C,
Sec.~\ref{sec:gap-register}).
...
merely permitted.  This is a separate argument from the self-modeling
...
The central claim of Part~A is that \emph{complexification is motivated
by the observer's algebraic nature, rather than assumed}.  We present
the physical argument in five steps; the formalization of this argument
as a precise theorem remains open (Gap~C).
...
we \emph{motivate} the complexification from the observer's C*-nature:
the self-modeling requirement (Paper~5) forces the observer to be a
C*-algebra system, which we argue motivates extension of scalars on
the spaces the observer probes.  The end result is the same
identification of $\Vhalf^{\,\mathbb{C}}$ as the Weyl spinor
$\Stenplus$, but the logical route is different: Boyle assumes
complexification; we give a physical argument for it (Gap~C).
\end{remark}
...
$\Spin{9}$-module, not automatically a $\Spin{10}$-module.  The key
fact is that the branching $\Stenplus|_{\Spin{9}} \cong S_9^{\,\mathbb{C}}$
is \emph{multiplicity-free}: $S_9^{\,\mathbb{C}}$ is irreducible under
...
The existence of this extension is guaranteed: $S_9$ is, by
construction, the restriction of a $\Spin{10}$ Weyl spinor
to $\Spin{9} = \mathrm{Stab}_{\Spin{10}}(e_{10})$.
Complexification therefore does not merely produce a representation
\emph{compatible} with $\Spin{10}$; it produces the unique
$\Spin{10}$-module extending the $\Spin{9}$ action, because the
representation space $S_9^{\,\mathbb{C}}$ already sits inside the
$\Spin{10}$ representation ring as an irreducible restriction.
\end{remark}
...
directly.  The results are consistent; our contribution is to
\emph{motivate} the complexification step from C*-observer nature
rather than taking it as a mathematical starting point.
\end{remark}

\begin{remark}[What the complexification argument does not yet
establish]\label{rem:complexification-limits}
The five-step argument above is \emph{physical reasoning}, not a
formal proof.  Two gaps remain:
\begin{enumerate}
  \item \textbf{$\mathbb{C}$-linearity does not force extension of
    scalars.}  A complex-algebraic observer can describe real
    configuration spaces, real observables, and real manifolds
    without complexifying them.  The step from ``the observer's
    internal scalar field is $\mathbb{C}$'' to ``every space the
    observer probes acquires a $\mathbb{C}$-module structure'' is
    plausible but requires a precise theorem about how
    C*-measurement maps interact with real target spaces.
  \item \textbf{Module extension is not symmetry extension.}  Even
    granting the vector-space complexification
    $\Vhalf\otimes_\mathbb{R}\mathbb{C}$, one must show that the
    $\Spin{9}$ action on the complexified space canonically extends
    to $\Spin{10}$.  This extension is unique by the
    multiplicity-free branching (Remark~\ref{rem:why-spin10}), but
    the physical justification for \emph{why} the larger symmetry
    is realized---rather than merely compatible---needs further
    development.
\end{enumerate}
A theorem of the form ``C*-measurement maps on a real Jordan module
necessarily factor through $\otimes_\mathbb{R}\mathbb{C}$'' would
close this gap.  We flag this as Gap~C.
\end{remark}
