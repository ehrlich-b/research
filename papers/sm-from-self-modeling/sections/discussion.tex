...
This paper completes a trilogy deriving the three pillars of
fundamental physics from the self-modeling principle.
...
entanglement entropy.  Combined with the entanglement equilibrium
argument~\cite{ConnesRovelli1994}, this yields Einstein's field
...
C*-observer nature rather than assuming it, (ii)~show that the same
...
(iii)~derive the complexification that provides the 16-dimensional
...
\medskip
\noindent\textbf{Dubois-Violette~\cite{DuboisViolette1995}.}
...
\begin{enumerate}
\item \textbf{Complexification derived from C*-observer nature (new).}
  The upgrade $\Spin{9}\to\Spin{10}$ and $\Ffour\to\Esix$ follows
  from the self-modeling requirement that the observer be a C*-algebra
  system (Paper~5~\cite{Paper5}), which forces extension of scalars on
  every space the observer probes.  This replaces the assumption of
  complexification in~\cite{Boyle2020} with a derivation.
