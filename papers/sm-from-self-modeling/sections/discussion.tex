...
\begin{enumerate}
  ...
  \item \textbf{Per-basin scope; cosmic gluing not addressed.}
    Theorem~\ref{thm:basin} (Restricted Basin) is per-basin: it
    identifies $\hthree$ as the unique observer-grade JA basin up to
    isomorphism, but does not determine whether the universe at
    large is a single $\hthree$ (with each observer occupying a
    different rank-1 idempotent), a sheaf of $\hthree$'s glued over
    a common spacetime, or an aggregate of observer-quanta each of
    which carries its own $\hthree$.  These three options
    ($\alpha_1$/$\alpha_2$/$\beta$) are explored informally in the
    program note ``U-B-M architecture'' but are outside the scope
    of the present paper.  The chirality and gauge-group results
    of Sections~\ref{sec:complexification}--\ref{sec:synthesis}
    apply to any single $\hthree$ basin and are unaffected by the
    cosmic-scale gluing question.
\end{enumerate}
