...
physically irrelevant (all choices are conjugate).  The third
(Gap~A) is the basin-identification step.  Two technical results
move it out of prose into formal statements: Theorem~\ref{thm:basin}
(Restricted Basin) packages the basin uniqueness as a
Definition/Theorem pair, and Lemma~\ref{lem:bottleneck} (C*-bottleneck
universality) packages the maximal C*-target structure that B2
selects within.  The complexification step (previously Gap~C) has
been reduced to a selection conditional on the observer's $T_a$-probing premise (Theorem~\ref{thm:complexification} supplies the mechanism; cf.~Sec.~\ref{sec:gap-register}).  This section
presents a detailed analysis of each.
...
\medskip
\noindent\textbf{Conditional on $T_a$-probing (formerly Gap~C):}
\begin{itemize}
  \item \textbf{L4:} The complexification mechanism is supplied, conditional on $T_a$-probing
...
\begin{itemize}
  \item \textbf{L2 (Gap~A):} Theorem~\ref{thm:basin} (Restricted
    Basin) identifies $\hthree$ as the unique observer-grade JA
    basin (Definition~\ref{def:basin}) up to isomorphism.  The proof
    routes through JvNW~\cite{JvNW1934}, BGW~\cite{BGW2020}, and
    Zelmanov~\cite{Zelmanov1979}.  The Level~IV realism step is
    recorded in Remark~\ref{rem:basin-scope}(b) as upstream
    motivation relating the anthropic ``observer-grade''
    restriction to the program's foundational premise; it is
    external to the proof.  Everything downstream (L3--L9) takes
    place within $\hthree$.
\end{itemize}
...
The complexification step (previously Gap~C) is supplied---conditional on the observer's $T_a$-probing premise---by the
...
A
  ...
  & \makecell[l]{Theorem~\ref{thm:basin} (Restricted Basin):\\
    $\hthree$ is the unique observer-grade JA basin\\
    via JvNW~\cite{JvNW1934} + BGW~\cite{BGW2020}\\
    + Zelmanov~\cite{Zelmanov1979}.\\
    The Level~IV realism step is now external\\
    to the proof, recorded in\\
    Remark~\ref{rem:basin-scope}(b) as the\\
    anthropic ``observer-grade'' restriction.}
  & HIGH
  & \makecell[l]{Deriving the ``observer-grade''\\
    restriction (Def.~\ref{def:basin})\\
    from self-modeling axioms,\\
...
C
  & Complexification (L4)
  & \makecell[l]{Conditional on $T_a$-probing (Theorem~\ref{thm:complexification} supplies the mechanism).\\
...
\begin{itemize}
  ...
  \item \textbf{A (HIGH):} Theorem~\ref{thm:basin} (Restricted
    Basin) identifies $\hthree$ as the unique observer-grade JA
    basin (Definition~\ref{def:basin}).  The proof
    chains JvNW~\cite{JvNW1934} (classification of simple FRJAs),
    BGW~\cite{BGW2020} (special FRJAs admit Jordan-tensor
    composites), and Zelmanov~\cite{Zelmanov1979} ($\hthree$ is the
    unique non-special simple FRJA).  The Level~IV mathematical
    realism step is no longer embedded in the proof: it is recorded
    in Remark~\ref{rem:basin-scope}(b) as the upstream motivation
    for the anthropic ``observer-grade'' restriction in
    Definition~\ref{def:basin}.  Without L4, the alternative
    formally real simple JAs (spin factors, $h_n(\mathbb{R})$,
    $h_n(\mathbb{C})$, $h_n(\mathbb{H})$) still exist as consistent
    mathematical structures and could in principle host other
    arenas; the program selects $\hthree$ as \emph{ours} by
    anthropic restriction, not by uniqueness across all possible
    worlds.  Closing Gap~A in the strong sense would mean deriving
    the observer-grade conditions of Definition~\ref{def:basin}
    directly from the self-modeling axioms, without anthropic input.
    Lemma~\ref{lem:bottleneck} (C*-bottleneck universality) closes
    one structural piece of this: condition~(ii) of
    Definition~\ref{def:basin} is now a theorem about
    h$_3(\mathbb{C}_u)$ as the maximal C*-target.
  \item \textbf{C (reduced to a selection):} The complexification mechanism (L4) is supplied, conditional on the observer's $T_a$-probing premise,
