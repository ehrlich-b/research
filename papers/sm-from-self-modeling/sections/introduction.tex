...
\begin{enumerate}
\item \textbf{Complexification argued, not assumed.} We argue that the
complexification $\Spin{9} \to \Spin{10}$ is motivated by the C*-algebra
nature of the observer (from self-modeling~\cite{Paper5}), rather than
imposed by hand as in~\cite{Boyle2020}.  The argument is physical rather
than a formal proof; making it rigorous is an open problem (Gap~C).
...
self-modeling framework. One link is \emph{argued} on physical grounds but
not formally proved (the complexification step, Gap~C). The remaining six
links are \emph{proved} within this work and its companions.

The result is conditional on four inputs: (i)~the identification of
$\hthree$ as the universe algebra via non-composability (Gap~A), (ii)~the
observer's choice of rank-1 idempotent $E_{11}$ (Gap~B, step~1),
(iii)~the choice of complex structure $u \in S^6$ (Gap~B, step~2), and
(iv)~a conjectural complexification principle (Gap~C) whose formalization
is an open problem (Sec.~\ref{sec:complexification-arg}).
Given these four inputs, the remaining links are proved.
...
Section~\ref{sec:complexification} (Part~A) argues that the C*-algebra nature
of the observer motivates complexification of the Peirce half-space $\Vhalf$,
upgrading $\Spin{9} \to \Spin{10}$ and $\Ffour \to \Esix$. This covers
links L4--L5 of the chain.  The argument is physical rather than a formal
proof (Gap~C).
