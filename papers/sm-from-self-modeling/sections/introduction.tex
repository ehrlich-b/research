...
real Jordan algebra that is \emph{non-composable}---it cannot be embedded as
a tensor factor of a larger Jordan algebra. Since the universe, by
definition, is not a subsystem of a larger system, $\hthree$ is the natural
candidate for the ``universe algebra'' whose internal structure should encode
...
L2 & Non-composability of $\hthree$ identifies it as the unique ``universe algebra''
   & JvNW~\cite{JvNW1934} + non-composability argument
   & \textit{Established} \\[4pt]
...
Dubois-Violette's earlier work~\cite{DuboisViolette1995}, and the broader
