...
\noindent The equality is at the Lie algebra level:
the intersection has $\dim = 12$, verified by explicit computation of
the common generators in~\cite{TodorovDrenska2018}, Sec.~4.  At the
group level, discrete quotients (the $\mathbb{Z}_3$ from
$\SU{3}_C\times\SU{3}_J/\mathbb{Z}_3$ and the $\mathbb{Z}_2$ from
$[\SU{2}\times\mathrm{U}(1)]/\mathbb{Z}_2$) affect the global
structure but not the gauge interactions.
...
\begin{enumerate}
  \item[\textup{(i)}] \textbf{The $\Ffour$ breaking.}\enspace
    ...
    gives $\SU{3}_C\times\SU{2}\times\mathrm{U}(1)$
    at the Lie algebra level
    \textup{($\dim = 12$;~\cite{TodorovDrenska2018}, Sec.~4)}.
...
\begin{itemize}
  \item Route~A: $\mathrm{U}(1)$ from the center of
    ...
    The generator acts as phase rotation on the off-diagonal
    octonion entries involving the first row/column of
    $h_3(\mathbb{C})\subset\hthree$, distinguishing the observer's
    slot ($E_{11}$) from the remaining two.
  \item Route~B: $\mathrm{U}(1)_Y$ with hypercharge
    $Y = (B\!-\!L) + 2J_3^R$.  The generator $B\!-\!L$
    comes from the $\mathrm{U}(1)$ factor in
    $\SU{4}\to\SU{3}_C\times\mathrm{U}(1)_{B-L}$, which is
    induced by the same $u$ that splits $\mathbb{O}$.
\end{itemize}
Both generators live in the rank-4 Cartan subalgebra of $\Ffour$
and are orthogonal to $\SU{3}_C$ and $\SU{2}$.  We do not independently derive the hypercharge formula; the explicit
identification of the Route~A generator with Route~B hypercharge
follows from Todorov and Drenska~\cite{TodorovDrenska2018}, who
verify that the $\mathrm{U}(1)$ center of $\mathrm{U}(2)_J$ acts
on the Peirce half-space $\Vhalf$ with the same eigenvalues as the
hypercharge $Y$ acts on $\Stenplus$.  This is a computation in
\cite{TodorovDrenska2018}, not just a rank count; our contribution
is the framework that determines the algebraic input, not the
hypercharge eigenvalue verification.\ $\therefore$
\textbf{Same $\mathrm{U}(1)$.}
...
\begin{tabular}{@{}lllp{4.5cm}@{}}
  ...
  $\mathrm{U}(1)$ & Center of $\mathrm{U}(2)_J$ & $\mathrm{U}(1)_Y$
    & Same: eigenvalues on $\Vhalf$ match~\cite{TodorovDrenska2018} \\
  \midrule
...
\begin{itemize}
  ...
  \item \textup{(B1)} An observer occupies a rank-1
    idempotent $e = E_{11}$, reducing $\Ffour\to\Spin{9}$
    (all idempotents $\Ffour$-conjugate; necessary symmetry
    reduction).
  \item \textup{(B2)} A complex structure
    $u\in S^6\subset\mathrm{Im}(\mathbb{O})$ is selected
    (necessary symmetry reduction forced by
    Theorem~\ref{thm:complexification} $+$ Schur;
    all $u$ $\Gfour$-conjugate).
\end{itemize}
Then:

\noindent\textup{(a)~\textbf{Gauge group.}}\enspace
The intersection of $\Spin{9} = \mathrm{Stab}_{\Ffour}(E_{11})$
with the $u$-preserving subgroup of $\Ffour$ is the SM gauge
group $\SU{3}_C\times\SU{2}\times\mathrm{U}(1)$ $(\dim = 12)$,
verified by explicit computation~\cite{TodorovDrenska2018}.
