...
as $\ln L$, not $L$.  For the Jacobson argument, sub-volume scaling
suffices (Sec.~\ref{sec:delta-s}).
...
The Jacobson argument requires sub-volume scaling of $\delta\ent$,
not a strict area-law bound on $\ent(A)$ itself
(Sec.~\ref{sec:delta-s}).
...
\item \textbf{Thermal states:} $I(A{:}B) \leq 2\beta|J|\,|\partial A|$
  (WVCH, no gap required, strict area law on mutual information).
\item \textbf{Ground states:} Sub-volume scaling
  ($\ent \sim \ln L$ in gapless 1D, strict area law in gapped phases).
  Not a strict area law in the gapless case, but sufficient for
  Jacobson.
\item \textbf{Perturbatively:}
  $\delta\ent \sim O(|\partial A|)$ (modular Hamiltonian locality,
  numerically verified).
\end{itemize}
The third perspective is the most directly relevant for the Jacobson
argument, which requires area-law scaling of $\delta\ent$, not of
$\ent$ itself.
