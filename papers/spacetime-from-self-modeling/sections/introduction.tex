...
continuum limit is standard methodology. No additional physical
assumptions are needed for Einstein's equations.
...
framework. We argue that in the present context it is
\emph{definitional}: in the absence of pre-existing geometry, the modular
...
the geometry-defining state by definition. This parallels the dissolution
of the involution in Paper~5~\cite{Paper5}: when there is no external
standard to match against, the structure produced by self-modeling
\emph{is} the standard. We note that Sorce~\cite{Sorce2024} has shown
...
definitional via the Connes--Rovelli thermal time
hypothesis~\cite{ConnesRovelli1994}; it does not appear as an assumption.
\label{tab:chain}}
\begin{ruledtabular}
...
L7 & Modular flow defines geometry (thermal time hypothesis)
   & Definitional & \ref{sec:equilibrium} & Reframed
   & Connes--Rovelli~\cite{ConnesRovelli1994} \\
L8 & $\Gab + \Lambda\,\gab = 8\pi\Gnewton\,\Tab$, \ $\Gnewton = 1/(4\eta)$
   & Derived from L1--L7 & \ref{sec:einstein} & Jacobson~\cite{Jacobson2016} applied
   & L1--L7 \\
\end{tabular}
\end{ruledtabular}
\end{table*}
...
a UV completion for Jacobson's argument, with L2--L4 being new results:
the forced Hamiltonian (L2), emergent causal structure (L3), and
area-law bounds (L4).  L1 is established in Paper~5~\cite{Paper5} and
L5 is a standard identity of quantum information theory.  Links L6--L8
follow Jacobson's derivation, with L7 reframed from an assumption to a
...
Section~\ref{sec:arealaw} establishes area-law entanglement from three
complementary perspectives: the Wolf--Verstraete--Cirac--Hastings (WVCH)
thermal mutual information bound~\cite{WVCH2008}, a channel-capacity
bound for pure states, and the entanglement first law combined with
modular Hamiltonian locality. Section~\ref{sec:equilibrium} presents the
thermal time hypothesis and its application to the self-modeling lattice,
dissolving the MVEH assumption into a definitional identification.
Section~\ref{sec:einstein} derives Einstein's equations by applying
Jacobson's entanglement equilibrium argument~\cite{Jacobson2016} to the
self-modeling lattice in the Wilsonian continuum limit.
