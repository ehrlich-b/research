...
results are \emph{consistent with} the analytical framework developed
in Secs.~\ref{sec:arealaw}--\ref{sec:einstein} and provide
\emph{supporting evidence}, but do not constitute proof.  All
entropies are computed in nats ($\ln$, not $\log_2$) and the modular
Hamiltonian is $\modH = -\ln\rhoA$.
...
\subsection{Modular Hamiltonian locality}
\label{sec:ed-ka}

The entanglement first law $\delta\ent = \delta\langle\modH\rangle$
(Sec.~\ref{sec:first-law}) reduces the Jacobson area-law input to the
locality of $\modH = -\ln\rhoA$.  We test this by expanding $\modH$
...
This strongly supports the modular Hamiltonian locality assumption
used in Sec.~\ref{sec:delta-s}, connecting directly to the Jacobson
input~(J1) via $\delta\ent \sim O(|\partial A|)$.
...
\subsection{Maximal vacuum entanglement hypothesis}
\label{sec:ed-mveh}

The MVEH---reframed as a structural identification in Sec.~\ref{sec:mveh}---predicts
that the self-modeling equilibrium state maximizes $\ent(A)$ among
nearby states.  We test this qualitatively by perturbing the
Hamiltonian: adding a random local field
$\delta H = \epsilon\, \hat{n}\cdot\vec{\sigma}_x$ at each site $x$,
recomputing the ground state, and measuring $\delta\ent(A)$.

For the Heisenberg AFM at $N = 12$ PBC with $A = \{0, 1\}$, using 20
random perturbation directions per site and three perturbation
strengths ($\epsilon = 0.05, 0.1, 0.2$):
\begin{itemize}
\item $100\%$ of all perturbations decrease $\ent(A)$
  ($\delta\ent < 0$), consistent with the unperturbed ground state
  being a local maximum of entanglement entropy.
\item The scaling is quadratic:
  $|\delta\ent(\epsilon = 0.2)|/|\delta\ent(\epsilon = 0.1)| = 3.76$
  (expected: $4.0$ for linear response), confirming that the
  perturbation is in the quadratic regime.
\end{itemize}
See Fig.~\ref{fig:mveh-check}.

These results are \emph{consistent with} MVEH but do not prove it:
the test is limited to $N = 12$, Hamiltonian perturbations (rather
than arbitrary state perturbations), and the $n = 2$ case.  The
structural identification of Sec.~\ref{sec:mveh}---that the vacuum is
identified as the state whose modular flow generates smooth
geometry---does not require numerical proof, but the numerical
evidence is reassuring.

\emph{Perturbation method note.}---The Heisenberg ground state is an
$SU(2)$ singlet; single-site unitary perturbations leave $\rhoA$
eigenvalues exactly invariant (the reduced state is $SU(2)$-symmetric
under local rotations), giving $\delta\ent = 0$ identically.
Hamiltonian perturbation, which breaks $SU(2)$ by adding a local
field, properly tests entropy maximality among nearby ground states.

\begin{figure*}
\centering
\includegraphics[width=\textwidth]{figures/fig_mveh_check.pdf}
\caption{MVEH numerical check (Heisenberg AFM, $N = 12$ PBC,
$A = \{0,1\}$).  (a)~Mean $\delta\ent$ for each site
$x \notin A$ at $\epsilon = 0.1$; all values are negative,
consistent with the unperturbed ground state being a local maximum of
$\ent(A)$.  (b)~$\langle|\delta\ent|\rangle$ versus $\epsilon$ on a
log-log scale; the dashed line shows quadratic scaling
$\propto\epsilon^2$.  Ratios of $3.76$ and $3.93$ are close to the
expected $4.0$.}
\label{fig:mveh-check}
\end{figure*}
...
$K_A$ SRF (Heisenberg) & $0.9993$ & ${>}\,0.5$ (strong pass) \\
MVEH $\delta\ent < 0$ fraction & $100\%$ & ${>}\,80\%$ (pass) \\
Quadratic ratio $|\delta\ent(2\epsilon)/\delta\ent(\epsilon)|$
  & $3.76$ & $4.0$ (linear response) \\
\end{tabular}
\end{ruledtabular}
\end{table}
