...
1. a **tracked-tree census** — over every *persisted* declaration whose defining
   ...
   modules on disk equals the set imported here *and* equals a pinned 187-name
...
run_cmd do
  ...
  -- equally), so the tracked surface is additionally pinned to this exact 187-name
  ...
  logInfo m!"Census PASS: {diskMods.length} tracked RadicalRelativity modules (== frozen 187-name manifest), custom axioms exactly {citedAxioms}, every tracked persisted declaration's closure ⊆ {allowed}"
