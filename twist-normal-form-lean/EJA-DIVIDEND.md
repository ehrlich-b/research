...
* **(E2) the Peirce decomposition** `J = ⊕ J_{ij}` for a Jordan frame, with the Faraut–Korányi
  ...
  ★★★ **"GATE (E2) IS DISCHARGED" DOES NOT MEAN THIS DELIVERABLE IS BUILT — 2026-08-22, and the two
  are routinely read as one sentence.** Two independent gaps, both established rather than suspected:
  **(a)** the frame-level `⊕_{i≤j} J_{ij}` is *still* not a declaration anywhere — the tree has the
  single-idempotent decomposition (`EJA/Peirce.lean:292`) and a pairwise split (`EJA/Block.lean:96`),
  per declaration lists of all 15 `EJA/*.lean` files; **(b)** `WallCertificates/eja-gated.lean`'s
  `gate_E2_peirce`, the theorem that discharged the gate, binds its `J2` and `ScalarOn`
  **existentially**, which makes its conclusion provably equivalent (machine-checked both directions)
  to a sentence carrying no Peirce vocabulary at all. So the gate certifies row 16's clause (1) and
  not clause (2). The strong statement *is* proved — in `EJA/InterfaceInstance.lean`, where those
  predicates are definitions — but the certificate does not reach it.
  ★ **The transferable rule: an existential over the predicates cannot certify anything about the
  intended predicates.** It was the right repair for the gate's earlier falsity and the wrong shape
  for evidence.
...
★★★ **ARC-9 UPDATE (2026-08-13) on row 16: the (E2) half is DISCHARGED on the concrete carrier.** `EJA/ConcreteInstance.lean` builds `ejaComparison` from `Necessity.comparisonSetup` plus the five equations, and `toCoalescenceSetup` produces a `CoalescenceSetup` on `H_N(ℂ)` with `simDiag_opCommute`, `aOf_scalarOn` and `block_mem_J2` **proved**. The row does not close: `Θ_fix` and `Θ_jordan` remain cited, so what is left of the gate is **(E3) only**. ★ The same work showed `WallCertificates/eja-gated.lean`'s `gate_E2_peirce` is under-hypothesised — its `haOf` does not tie the coefficients to `r`. ★★★ **THAT DEFECT WAS REPAIRED ON 2026-08-20 AND THIS SENTENCE HAS ASSERTED IT IN THE PRESENT TENSE EVER SINCE — CORRECTED 2026-08-22.** The hypothesis now reads `haOf : ∀ r, C.aOf r = ∑ i, Real.exp (r i) • C.p i`, so the coefficient function **is** `fun i => Real.exp (r i)`: there is no free `c` left to float away from `r`, and `r i = r j → c i = c j` is one `congrArg Real.exp` (`EJA/InterfaceInstance.lean:157` is that step). **The defect is closed, not relabelled**, and the gate is a proved theorem. ★★ **Note how this survived: the sentence next to it was rewritten today by the correction pass, and this one inherited the freshness of its neighbour.** "Fix the row, not just the footnote" running in the direction that is hardest to see — the stale clause sat *inside* the block being edited, one sentence from the new text. ★★★ **"what is left of the gate is (E3) only" IS WITHDRAWN 2026-08-22.** The sentence names the right two survivors — `Θ_fix` **and** `Θ_jordan` — and then collapses them into the wrong one. `Θ_jordan` is (E3) and this row does not consume it; `Θ_fix` is vdW Prop 5.5 span-extended by `lem:span` (manifest row 5) and is not a gate at all. The clause that is true is the first one: the row does not close.
