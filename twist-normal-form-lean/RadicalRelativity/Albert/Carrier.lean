...
`off 2 = (1,2)`.  This is the convention of the older expository `RadicalRelativity/Albert.lean`
of the source tree (`~/repos/research/lean/`), which is **not** part of this tree: it still
carries a `sorry` at its own `jordan_identity` and was deliberately left behind.  The convention
is kept only so that its frame lemmas would transfer if they were ever ported.

## Octonions as an `ℝ`-module

`RadicalRelativity/Octonions.lean` gives `Octonion` bare `Zero`/`Add`/`Neg`/`SMul ℝ` and `Mul`
instances and no bundled algebraic class (no `AddCommGroup`, no `Module`).  The first section
here supplies
...
discharged by anything in the `Albert/` layer: that row says an arbitrary S1--S7 sequential
product on `h₃(𝕆)` must be Lüders, and what these files build is the carrier and its algebraic
structure, not a statement about products on it.
-/
