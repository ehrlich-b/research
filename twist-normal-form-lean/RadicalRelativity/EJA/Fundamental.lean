...
★★ **MEASURED NEGATIVE RESULT, recorded so nobody retries the cheap route.**  (FF1) is **not** a
linear combination of degree-5 instances of the linearised Jordan identity.  A search over 390
relations — every `lin_jordan_apply` instance with one argument a two-generator product, every
degree-4 instance multiplied by a generator, the raw Jordan identity `(ab)a² = a(ba²)` in both
those shapes, and full commutativity `uv = vu` at every product node — gives an **inconsistent**
linear system over `ℚ`.  The degree-5 part of the ideal generated by the Jordan axioms does not
contain it.

That is the formal reason the fundamental formula is a multi-page derivation in every textbook
treatment rather than a one-line computation: it needs the *operator* chain with genuine
intermediate lemmas, not a linear-algebra shortcut over the linearised identity.
