...
| 31 `thm:qubit-boundary` | **PARTIAL — demoted** | clause (ii): `RankTwo.n2SequentialProduct RankTwo.tauModuliRP2` incl. S2; clause (iii): `RankTwo.not_exists_jordanAuto_const_twist` in the article's literal `(Φ,t)` form, resting on `Necessity.jordanAuto_orderAuto` / `preservesJordan_iff_orderAuto`, plus `not_forall_effects_tau_eq_twistSeq` at the effects | ★★★ **THIS FILE WAS RIGHT AND THE MANIFEST WAS WRONG.** The clause-(i) cell in §1 names the *entry-level* witness `sp_blockForm`, which is exactly the demotion's evidence: clause (i)'s coherence-action half is part of the article's statement and does not exist in the `twistSeq` encoding. The manifest's promotion inherited `WallCertificates/frame-geometry.lean:44` ("Parts (i) and (iii) are proved"), written before the row-30/31 encoding split was known and closed later for clause (iii) only. Residue: **assembly** of `n2Sp_eq_twistSeq_at_frame`, `twistSeq_adU_mat`, `twistSeq_diagFamily_blockHerm`, `orientationJ` — not new mathematics |
