...
| 22 `lem:orientation` | **PARTIAL** | `Necessity.IsCrossCoherent` (`ComplexRowUnconditional.lean:915`), `isCrossCoherent_iff`, `orientationJ` (`:880`) with `orientationJ_corner`, `_isCrossCoherent`, `_sq`, `_adU`, `orientationJ_mat_eq`, `adU_eq_rotation_on_crossCoherent` | ★★★ **This file had NO entry for row 22 at all** — one of the labels §3b was created to stop happening, recurring. Clauses (i) and (ii) are proved at the article's generality (orthogonal idempotents, arbitrary index type), (ii) *unrestricted* and so stronger than the article's; clause (i)'s "independent of the splitting `q = p₁ + p₂`" phrase has **no theorem** and deliberately so — `orientationJ` takes `q` and `p` and nothing else, so every Lean rendering is `rfl`. Read the section header at `ComplexRowUnconditional.lean:805` before repricing |
