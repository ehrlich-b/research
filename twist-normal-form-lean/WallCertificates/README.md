...
| `hurwitz-classification.lean` | **no manifest row** — 2026-08-23, the composition-algebra block. ★★★ **DISCHARGED 2026-08-23, the same day it was written, zero `sorry`.** `Composition/Classification.lean` proves `CompositionAlgebra.hurwitz_classification` outright and in a **strictly stronger form** than the certificate stated — the certificate's `IsCompIso` carried unit and product only, and said in so many words that "anyone who closes it should also add the norm clause"; the proved one carries `map_nf`. The certificate is kept in the shape of `eja-power-assoc.lean`: statement verbatim, `sorry` replaced by the derivation of the weak form from the strong one, original text kept with six corrections tagged `[D1]`…`[D6]`. **The two most useful are opposite in sign.** `[D4]`: the certificate's one *correct* prediction was naming the octonion identification "the one to worry about" — it was the only step needing a search outside Lean (1344 of 2744 monomial correspondences work; **no sign-free one exists** — a claim about
monomial correspondences only, since `Aut(𝕆) = G₂` is 14-dimensional), and the search was run in Python against the transcribed Fano table and verified before a line of Lean. `[D2]`: its estimate of that step's *shape* was wrong in the safe direction — "64 basis-pair equations, each plausibly `rfl`-or-`decide`" is wrong on both count and tactic, since the coefficients are real (`decide` cannot apply, `rfl` cannot either) and the proof is 8 coordinate goals closed by `simp` then `ring`. `[D6]` records the obstacle nobody predicted and the only one that cost time: Mathlib's `Quaternion.re_mul` does not fire when its first argument is a **structure literal**, so the natural `simp` form of the conjugation silently blocked the whole computation. ★★ Two of its claims were already false when first written and are corrected in place: a Mathlib grep count quoted from memory (16/twelve, actually 15/11 — **still 15/11 at the discharge, re-run**), and an in-tree absence claim that "returns nothing" when the grep returned one hit. `[D1]` is the third: "the tree has no `CD` type former and no composition-algebra isomorphism type" returned 1 hit when written and **26** at the discharge |
| `jacobson-coordinatization.lean` | **no manifest row** — 2026-08-23, the Jordan-side half of the JvNW campaign. The coordinate algebra IS built in-tree (`EJA/Connection.lean`, `EJA/Coordinatize.lean`, `EJA/CoordinatizeWitness.lean`, 1,047 lines, axioms `[]`): an off-diagonal frame block carries a `CompositionAlgebra` structure, so Hurwitz applies: `dim V_ij ∈ {1,2,4,8}` and — chaining `Composition/Classification.lean`, which landed the same morning — `V_ij` is *isomorphic* as a composition algebra to `ℝ`, `ℂ`, `ℍ` or `𝕆`. A `CoordData` is exhibited on `H_d(ℂ)`, so the layer is not about an empty structure. Three residues carry a `sorry` each: the isomorphism `J ≅ H_n(C)`, **simple ⟹ every block nonzero** (the tree has no Jordan-ideal notion at all, so the certificate defines one), and `n ≥ 4` ⟹ the coordinate algebra is associative. ★★ **Residue 1 is DISCHARGED (2026-08-23)**: the certificate said `H_n(C)` does not exist as a type, and `RadicalRelativity/Composition/HermMat.lean` now builds it — a `Submodule ℝ (Matrix ι ι C)` with the entrywise hermitian condition (the vendored `HermitianMat` still cannot be reused, and `CompositionAlgebra` still carries no `Star` instance, so the condition is entrywise through `cstar`), the symmetrised product closed on it, the diagonal frame and off-diagonal blocks, transport along `IsCompIso`, and the Jordan identity for **associative coefficients only** — so `H_n(𝕆)` is NOT shown to be a Jordan algebra at any rank. The `sorry` on `jacobson_coordinatization` is untouched. ★ Residue 3 remains **unpriced** — it was not attempted. ★★ The build itself is the pricing datum: the campaign plan called this its only zero-anchor module at 1,500–3,500 lines and 40% of the whole campaign, and what got built came to 1,047 lines including a carrier the plan did not ask for, with the two load-bearing identities compiling on the first or second attempt |
